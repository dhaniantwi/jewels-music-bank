...
export default function App() {
  ...
  const handleDeleteSong = (songId: number) => {
    ...
    // Remove deleted song from ministrations
  ...
  const handleSaveMember = (
    memberData: Omit<TeamMember, 'id'> & { id?: number }
  ) => {
    if (memberData.id) {
  ...
  const handleDeleteMember = (memberId: number) => {
    ...
    // Remove member from lead assignments
    setMinistrations((prev) =>
      prev.map((ministration) => ({
        ...ministration,
        songs: ministration.songs.map((item) =>
          item.lead === memberId
            ? {
                ...item,
                lead: null
