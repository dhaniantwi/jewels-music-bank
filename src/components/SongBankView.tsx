...
import { Song, SongCategory, ActiveRole } from '../types';
import { Search, Plus, Play, Pause, Filter, Music, Sparkles, SlidersHorizontal, Trash2, Edit } from 'lucide-react';
import { CHROMATIC_KEYS, playPitchTone } from '../utils/audioUtils';
...
export const SongBankView: React.FC<SongBankViewProps> = ({
  ...
  const categories: SongCategory[] = ['All', 'Worship', 'Praise', 'Gospel', 'Afropraise', 'Medleys', 'Choir', 'Contemporary'];
  ...
  const filteredSongs = useMemo(() => {
    return songs.filter(song => {
      ...
      const matchesKey = selectedKey === 'All' || song.key.toLowerCase() === selectedKey.toLowerCase();

      return matchesSearch && matchesCategory && matchesKey;
    });
  }, [songs, searchTerm, selectedCategory, selectedKey]);

  const handleQuickPlay = (e: React.MouseEvent, song: Song) => {
    e.stopPropagation();
    ...
    playPitchTone(`${song.key.replace('m', '')}4`, 3);
    setTimeout(() => {
      setPlayingSongId(prev => prev === song.id ? null : prev);
    }, 3000);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6e6e73] font-medium mt-0.5">
            {songs.length} ministry songs loaded with full lyrics, chord charts, vocal harmonies, and MD notes.
          </p>
        </div>
      ...
      <div className="ios-glass rounded-[28px] p-4 sm:p-5 bg-white/80 space-y-3.5 border border-white/90 shadow-sm">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white/90 border border-black/10 rounded-2xl pl-11 pr-4 py-3 text-sm text-[#1d1d1f] placeholder:text-[#86868b] outline-none focus:border-[#007aff] focus:ring-2 focus:ring-[#007aff]/10 transition-all font-medium"
          />
          {searchTerm && (
            <button
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
          ...
          <div className="flex items-center gap-2 flex-shrink-0 self-end sm:self-auto">
            <span className="text-xs font-bold text-[#86868b]">Key:</span>
            <select
              value={selectedKey}
      ...
      {filteredSongs.length > 0 ? (
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-1.5 my-2 p-2 rounded-xl bg-black/[0.025] text-center text-[10px] font-semibold text-[#6e6e73]">
                  <div className="truncate">👑 Lead: {song.arrangement?.lead ? 'Ready' : '—'}</div>
                  <div className="truncate text-[#7c3aed]">🎶 SAT Harmonies</div>
                  <div className="truncate">🎹 Band Chart</div>
                </div>
                ...
                <div className="flex items-center justify-between pt-3 mt-1 border-t border-black/5">
                  ...
                  >
                    ...
                    <span>{isPlayingThis ? 'Playing Key...' : 'Key Tone'}</span>
                  </button>
        ...
        <div className="text-center py-16 px-4 ios-glass rounded-[32px] bg-white/70">
          ...
          <h3 className="text-xl font-bold text-[#1d1d1f]">No songs found</h3>
          <p className="text-sm text-[#6e6e73] mt-1 max-w-sm mx-auto">
            Try searching for another song title or artist, or clear your active filters.
          </p>
